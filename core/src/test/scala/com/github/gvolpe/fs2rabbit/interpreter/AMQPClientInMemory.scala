...
class AMQPClientInMemory(internalQ: mutable.Queue[IO, Either[Throwable, AmqpEnvelope]],
                         ...
                         config: IO[Fs2RabbitConfig]) extends AMQPClient[Stream[IO, ?]] {
  ...
  override def basicConsume(channel: Channel, queueName: model.QueueName, autoAck: Boolean, consumerTag: String, noLocal: Boolean, exclusive: Boolean, args: Map[String, AnyRef]): Stream[IO, String] = {
    val ifError = Stream.raiseError[String](new java.io.IOException(s"Queue ${queueName.value} does not exist!")).covary[IO]
  ...
  override def deleteQueue(channel: Channel, queueName: model.QueueName, ifUnused: Boolean, ifEmpty: Boolean): Stream[IO, Unit] = Stream.eval(IO(queues -= queueName)).map(_ => ())
  override def deleteQueueNoWait(channel: Channel, queueName: model.QueueName, ifUnused: Boolean, ifEmpty: Boolean): Stream[IO, Unit] = Stream.eval(IO(queues -= queueName)).map(_ => ())
  override def bindQueue(channel: Channel, queueName: model.QueueName, exchangeName: model.ExchangeName, routingKey: model.RoutingKey): Stream[IO, Unit] = Stream.eval(IO.unit)
  override def bindQueue(channel: Channel, queueName: model.QueueName, exchangeName: model.ExchangeName, routingKey: model.RoutingKey, args: model.QueueBindingArgs): Stream[IO, Unit] = Stream.eval(IO.unit)
  ...
  override def unbindQueue(channel: Channel, queueName: model.QueueName, exchangeName: model.ExchangeName, routingKey: model.RoutingKey): Stream[IO, Unit] = Stream.eval(IO.unit)
  override def bindExchange(channel: Channel, destination: model.ExchangeName, source: model.ExchangeName, routingKey: model.RoutingKey, args: model.ExchangeBindingArgs): Stream[IO, Unit] = Stream.eval(IO.unit)
  override def declareExchange(channel: Channel, exchangeName: model.ExchangeName, exchangeType: ExchangeType): Stream[IO, Unit] = Stream.eval(IO.unit)
  override def declareQueue(channel: Channel, queueName: model.QueueName): Stream[IO, Unit] = Stream.eval(IO(queues += queueName)).map(_ => ())
}