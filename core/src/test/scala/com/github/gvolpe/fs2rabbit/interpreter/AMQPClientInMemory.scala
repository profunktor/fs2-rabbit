...
class AMQPClientInMemory(internalQ: mutable.Queue[IO, Either[Throwable, AmqpEnvelope]],
  ...
  override def deleteQueue(channel: Channel, queueName: model.QueueName, ifUnused: Boolean, ifEmpty: Boolean): Stream[IO, Unit] = Stream.eval(IO.unit)
  ...
  override def declareQueue(channel: Channel, queueName: model.QueueName): Stream[IO, Unit] = Stream.eval(IO.unit)
}